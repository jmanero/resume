...
class Style {
  ...
  static resolve(path, parent) {
    Log.debug(`Resolve SCSS module ${path}` + (parent ? `, imported by ${parent}` : ''));
    ...
    if (this.includes.has(name)) {
      ...
      Log.debug(`Using module ${name} (${prefix}) for ${path}`);
    ...
    Log.debug(`Using module ${resolved.module.name} (${resolved.module.path}) for ${file}`);
