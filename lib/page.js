...
class Page {
  ...
  static serve() {
    return function(req, res, next) {
      ...
      Log.info(`PAGE: Serve ${path} for ${req.originalUrl}`);
  ...
  /**
   * Render pages into a static site directory
   * @return {Function}
   */
  static static() {
    return function(writer) {
      return File.resources('pages', '.yml')
        .then((files) => Promise.all(files.map((page) => {
          return Page.fromYAML(page)
            .then((page) => page.render())
            .then((content) => writer.file(page, content, '.html'));
        })));
    };
  }
